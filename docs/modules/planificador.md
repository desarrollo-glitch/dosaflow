...
- **Integración con IA externa:** Los procesos de bitácora y reuniones requieren una API Key válida (expuesta como `VITE_GEMINI_API_KEY` en `.env.local`) para comunicarse con Gemini (por defecto 2.5 Flash, configurable con `VITE_GEMINI_MODEL`). La salida JSON se valida y, si es correcta, se transforma en documentos Firestore.
...
- **Variables de entorno:** `.env.local` debe exponer la API Key de Gemini mediante `VITE_GEMINI_API_KEY` y, opcionalmente, el identificador de modelo (`VITE_GEMINI_MODEL`, por defecto `gemini-2.5-flash`) para habilitar los procesamientos automáticos.
