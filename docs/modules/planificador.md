...
- **Integración con IA externa:** Los procesos de bitácora y reuniones requieren una API Key válida (expuesta como `process.env.API_KEY`/`GEMINI_API_KEY` en `.env.local`) para comunicarse con Gemini 2.5 Flash. La salida JSON se valida y, si es correcta, se transforma en documentos Firestore.
...
- **Variables de entorno:** `.env.local` debe exponer la API Key de Gemini (mapeada a `process.env.API_KEY` en `src/App.tsx`) para habilitar los procesamientos automáticos.
