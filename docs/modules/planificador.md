...
- **Integración con IA externa:** Los procesos de bitácora y reuniones requieren una API Key válida (expuesta como `VITE_GEMINI_API_KEY` en `.env.local`) para comunicarse con Gemini (por defecto 2.5 Flash, configurable con `VITE_GEMINI_MODEL`). Opcionalmente se puede exponer `VITE_OPENAI_API_KEY`/`VITE_OPENAI_MODEL` y definir una cadena de proveedores en `VITE_AI_PROVIDER_CHAIN` para que el cliente pruebe varios motores hasta obtener una respuesta JSON válida. La salida se valida antes de persistirla en Firestore.
...
- **Variables de entorno:** `.env.local` debe exponer la API Key de Gemini mediante `VITE_GEMINI_API_KEY` y, opcionalmente, el identificador de modelo (`VITE_GEMINI_MODEL`, por defecto `gemini-2.5-flash`). Para habilitar el fallback con OpenAI se añaden `VITE_OPENAI_API_KEY`, `VITE_OPENAI_MODEL` y (si se desea controlar el orden) `VITE_AI_PROVIDER_CHAIN` con valores del tipo `gemini:gemini-2.5-flash,openai:gpt-4o-mini`.
